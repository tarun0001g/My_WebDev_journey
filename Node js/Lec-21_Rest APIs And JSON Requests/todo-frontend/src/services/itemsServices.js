...
export const addItemToServer = async (task, date) => {
  const response = await fetch("http://localhost:3007/api/todo", {
    method: 'POST',
    headers: {
...
export const markItemCompletedOnServer = async (id) => {
  const response = await fetch(`http://localhost:3007/api/todo/${id}/completed`, {
    method: 'PUT',
    headers: {
      "Content-Type": "application/json"
    }
...
export const deleteItemFromServer = async (id) => {
  const response = await fetch(`http://localhost:3007/api/todo/${id}`, {
    method: 'DELETE'
  });
  return id;
}
