...
const sumRequestHandler = (req, res) =>{
  ...
  const body = [];
  req.on('data', (chunk) => { //This is a async event listener, it will executed only when data chunk starts to arrive
    body.push(chunk);//pushed all data chunks in body array
  } )
  
  req.on('end',() => { //this is async event listener, it will executed when all chunks are arrived.
    //Note:- body means data.
    const bodyStr = Buffer.concat(body).toString(); //parsing data chunks in string form  at
    ...
    console.log(result);
    //we will put result in this "end" event listener because, we need complete data chunks to generate the result
